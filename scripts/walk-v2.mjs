...
//   場の傾向（時点つき）をここで足す。
const rfeatCols = (() => {
  try { return all(`PRAGMA table_info(rfeat)`).map((c) => c.name).filter((c) => c !== 'race_id' && c !== 'lane') }
  catch { return [] }
})()
...
// ★2着・3着を選ぶときに「先に決まった艇」を入力する。
//   これまでは1着の艇を候補から外すだけで、誰が1着だったかを渡していなかった。
//   実測（condorder.mjs・22万レース）では最大21.0ptのズレがある：
//     1着が2号艇 → 2着に1号艇 44.2%（1着を除いただけの予測なら23.1%）
//     1着が3号艇 → 2着に1号艇 39.3%（同 22.2%）
//     1着が5号艇 → 2着に1号艇 35.6%（同 19.8%）
//   外枠が1着になると1号艇が2着に残る、という競艇の型をモデルが知らなかった。
const EX = 16
const DT = D + EX
// ex[0..5]  先に1着になった艇の枠（該当枠が1）
// ex[6..11] 先に2着になった艇の枠
// ex[12]    1着の艇と自分の枠の差（-1〜1に正規化）
// ex[13]    自分が1着の艇より内側なら1
// ex[14,15] 2着の艇について同じ
const exVec = (myLane, l1, l2) => {
  const e = new Float64Array(EX)
  if (l1) { e[l1 - 1] = 1; e[12] = (myLane - l1) / 5; e[13] = myLane < l1 ? 1 : 0 }
  if (l2) { e[6 + l2 - 1] = 1; e[14] = (myLane - l2) / 5; e[15] = myLane < l2 ? 1 : 0 }
  return e
}

function fitPL(races, { lr = 0.25, epochs = 70, l2 = 3e-4 } = {}) {
  const W = [0, 1, 2].map(() => new Float64Array(DT))
  const MM = [0, 1, 2].map(() => new Float64Array(DT))
  const VV = [0, 1, 2].map(() => new Float64Array(DT))
  for (let ep = 1; ep <= epochs; ep++) {
    const G = [0, 1, 2].map(() => new Float64Array(DT))
    for (const race of races) {
      ...
      for (let st = 0; st < 3; st++) {
        ...
        // 先に決まった艇（st=0 なら無し／st=1 は1着／st=2 は1着と2着）
        const prev1 = st >= 1 ? race.ord[0] : 0
        const prev2 = st >= 2 ? race.ord[1] : 0
        const exs = cand.map((i) => exVec(bs[i].lane, prev1, prev2))
        const s = cand.map((i, k) => {
          let z = 0; const x = bs[i].x
          for (let d = 0; d < D; d++) z += w[d] * x[d]
          const e = exs[k]; for (let d = 0; d < EX; d++) z += w[D + d] * e[d]
          return z
        })
        const mx = Math.max(...s)
        ...
        for (let k = 0; k < cand.length; k++) {
          ...
          const e = exs[k]; for (let d = 0; d < EX; d++) g[D + d] += coef * e[d]
        }
        gone.add(pick)
      }
    }
    const n = races.length
    for (let st = 0; st < 3; st++) for (let d = 0; d < DT; d++) {
...
function trioProbs(W, race) {
  const bs = race.boats
  // 本体の点数（条件に依らない部分）は1回だけ計算して使い回す
  const base = [0, 1, 2].map((st) => bs.map((b) => {
    let z = 0; const x = b.x
    for (let d = 0; d < D; d++) z += W[st][d] * x[d]
    return z
  }))
  // ★条件つきの上乗せ。誰が先に決まったかで変わるので、その都度足す
  const addEx = (st, i, l1, l2) => {
    const e = exVec(bs[i].lane, l1, l2)
    let z = 0
    for (let d = 0; d < EX; d++) z += W[st][D + d] * e[d]
    return z
  }
  const soft = (idx, sc) => {
  ...
  for (let a = 0; a < N; a++) {
    ...
    // 1着が a だと分かった前提で2着の点数を出し直す
    const s2 = []
    s2[0] = 0
    const sc2 = new Array(N).fill(-Infinity)
    for (const i of i2) sc2[i] = base[1][i] + addEx(1, i, l1, 0)
    const q2 = soft(i2, sc2)
    for (let bi = 0; bi < i2.length; bi++) {
      const b = i2[bi], l2 = bs[b].lane
      const i3 = idxAll.filter((i) => i !== a && i !== b)
      // 1着と2着が分かった前提で3着の点数を出し直す
      const sc3 = new Array(N).fill(-Infinity)
      for (const i of i3) sc3[i] = base[2][i] + addEx(2, i, l1, l2)
      const q3 = soft(i3, sc3)
      for (let ci = 0; ci < i3.length; ci++)
        out.push({ combo: `${l1}-${l2}-${bs[i3[ci]].lane}`, p: p1[a] * q2[bi] * q3[ci] })
    }
  }
