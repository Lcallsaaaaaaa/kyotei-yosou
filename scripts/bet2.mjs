// 市場と「着順全体モデル(pred3)」を混ぜて、買える戦略を探す。
//
...
/**
 * 指定区分のレースを、120通りの {市場確率, モデル確率, オッズ, 的中} に展開する。
 * ★ model4の1着確率をPlackett-Luceで機械展開する方式から、
 *   model5が直接出した120通りの確率(pred3)を使う方式に変えた。
 *   機械展開では3連単の組単位で市場に何も足せなかった（係数0.066）。
 */
function load(split) {
  const om = new Map()
  ...
  for (const [rid, m] of om) {
    const pm = mdl.get(rid); if (!pm || m.size < 100 || pm.size < 100) continue
    let tot = 0
